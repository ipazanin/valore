...
- Update names, balances, quantities, prices, estimates, and outstanding amounts. Investment identity and existing record categories remain fixed. To record a zero holding or repaid debt, update its quantity or amount to zero. Deleting every observation keeps the named record, which then has no recorded value and is excluded from totals. Price deletion may leave positive holdings unvalued. Archive accounts, holdings, assets, or debts from a chosen past or present date. History before closure remains intact; values are excluded on and after closure. Closing an account requires an explicit zero cash balance and zero quantities for every holding, including archived holdings. Later observations must be corrected first. Sale proceeds and repayments remain separate manual cash updates. Removing a closure corrects history and restores carried-forward values; it is not a dated reopening.
