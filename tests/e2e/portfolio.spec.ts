...
test('backup validation and preview preserve records until replacement; restore works in a fresh context', async ({
  ...
  await expect(page.getByText('Validated backup · Version 4')).toBeVisible()
  ...
  await expect(page.getByText('Validated backup · Version 4')).toHaveCount(0)
  ...
  try {
    ...
    await expect(onboardingPage.getByText('Validated backup · Version 4')).toBeVisible()
  ...
  try {
    ...
    await expect(otherPage.getByText('Validated backup · Version 4')).toBeVisible()
    await expect(
      otherPage.getByText('This replaces every current record and the currency and appearance settings.'),
    ).toBeVisible()
